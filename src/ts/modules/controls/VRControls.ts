...
interface VRControls {
  ...
  leftThumbstickInertia: { val: number },
  rightThumbstickInertia: { val: number },
  leftThumbstickMomentum: { val: number },
  rightThumbstickXMomentum: { val: number },
  rightThumbstickYMomentum: { val: number },
  thumbstickMax: number,
  controllerModelFactory: XRControllerModelFactory,
  leftControllerWorldData?: { position: THREE.Vector3, rotation: THREE.Quaternion, scale: THREE.Vector3 },
  ...
  buttons: {
    [key in 'left' | 'right']: {
      [key: string]: { previousFrame: undefined | boolean, thisFrame: undefined | boolean, buttonUp: undefined | boolean },
    }
  },
...
class VRControls {
  ...
  update() {
    ...
    const buttons: { [key: string]: number } = { trigger: 0, a: 4, b: 5 };
    sides.forEach((side) => {
    ...
    if (this.controllers.left && this.controllers.left.gamepad) {
      ...
      if (move.y !== 0) {
        ...
        if (typeof this.controls.floorLimit === 'number' && (this.controls.gravity === true || moveYResult < minPlayerY)) {
  ...
  handleControllerIntersections(controller: Controller) {
